...
export default class Checklist {
  public async consultar(chaveNfe: string): Promise<TextoChecklist> {
    ...
    if (parametros) {
      ...
      if (parametros.checkModelo == "conecta") { //multilazer e mvx
        ...
        const motivo = await this.processarRegex(itensSelecionadosRegex, texto.motivo)
        texto.motivo = motivo
      }
      else if (parametros.checkModelo == "novo_conecta") { //multilazer e mvx
        ...
        texto = await this.ler(motivoRegex, detalhesRegex, url)
        if(texto.detalhes === 'CAMPOS PARA VALIDAÇÃO DO MOTORISTA OU CONFERENTE') texto.detalhes = ''
      }
  ...
  private async ler(motivoRegex: RegExp, detalhesRegex: RegExp, url: string): Promise<TextoChecklist> {
    const pdf = new Pdf
