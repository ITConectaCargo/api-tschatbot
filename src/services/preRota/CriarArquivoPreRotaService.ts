...
export default class CriarArquivoPreRotaService {
  ...
  public async arquivoBot(
    excel: string[][],
    dataAgendamento: moment.Moment,
  ): Promise<DadosFinais> {
    ...
    for (let i = 1; i < excel.length; i++) {
      ...
      const infoAdicional: string = dadosPortal[0]?.info_adicional;
      ...
      for (let i = 0; i <= todosTelefones.length; i++) {
        if (todosTelefones[i])
          todosTelefones[i] = limparTelefone(todosTelefones[i]);
      }

      if (
        ocorrencia === 'A agendar' ||
        ocorrencia === 'A Reagendar' ||
        ocorrencia === 'Quebra do Veiculo' ||
        ocorrencia === 'Falta de Tempo Hábil' ||
        ocorrencia === 'Bau Cheio'
      ) {
        ...
        for (let i = 0; i < todosTelefones.length; i++) {
          ...
          if (telefone && !telefonesAdicionados.includes(telefone)) {
            const padraoBot: string[] = [
              ...
              cep,
              complemento,
              dataAgendamento.format('DD/MM/yyyy'),
              ocorrencia,
            ];
    ...
    const nomeArquivoVerificar = `Verificar-${dataAgendamento.format(
      'DD-MM-yyyy',
    )}`;
    ...
    const localBot = await excelSevice.criaPlanilha(
      colunas,
      celulares,
      pastaRelatorios,
      nomeArquivoCel,
    );
    const localVerificar = await excelSevice.criaPlanilha(
      colunas,
      verificar,
      pastaRelatorios,
      nomeArquivoVerificar,
    );
