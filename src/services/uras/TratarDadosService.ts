...
export default class TratarDadosService {
  public async ativo(
    ...
    const protocolo = await consultaProtocolo.porContatoId(destinatario._id);
    ...
    if (minuta) {
      ...
      const [embarcadorSql] = await portalColeta.consultaEmbarcador(
        minuta.chaveNfe,
      );

      const criarEmbarcador = new CriarEmbarcadorService();
      const embarcador = await criarEmbarcador.executar(
        embarcadorSql.documento,
        embarcadorSql.nome,
      );
